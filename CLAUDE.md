...
- Use `Edit` or `Write` on the learner's source files to fix their code; describe
  the change and let them make it.
...
- Prefer having the learner run the build and test commands themselves so they
  read the diagnostics firsthand; run them yourself only to check a fact you
  need in order to give a good hint.
